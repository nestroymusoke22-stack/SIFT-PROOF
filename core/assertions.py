...
def run_assertion(claim_json):
    ...
    # ── ALL valid evidence tables — disk + memory ─────────────────────────────
    valid_tables = [
        # Disk artifact tables
        'mft_events', 'amcache_entries', 'prefetch_events',
        'evtx_events', 'registry_runkeys',
        # Memory artifact tables
...
def _run_temporal_checks(claim_json, rows):
    ...
    result = {
        "contradiction": False,
        "description": "", "event_a": "", "event_b": "", "sequence": ""
    }
    ...
    # Check 3: User activity before account creation
    if table == 'evtx_events':
        for row in rows:
            user = row.get('username', '')
            ts   = row.get('timestamp', '')
            if not user or not ts or user.upper() in (
